// Run with: node scripts/import-blogs-content.js [--dry]
//
// Imports the finished articles in "blogs_content/" into the blogs table,
// converting each .docx straight to HTML. Nothing is authored here: the
// title, body, meta title, meta description and slug all come from the
// document itself. Matching is by slug, so re-running updates the existing
...
const SOURCE_DIR = path.join(ROOT, "blogs_content");
const WORK_DIR = path.join(require("os").tmpdir(), "blogs-content-docx");
const DRY_RUN = process.argv.includes("--dry");

// "Bridging the Gap Between DevOps and Finance Teams with FinOps.docx" contains a
// mismatched draft of the "AI vs Traditional Cost Management" article instead of its
// own topic (confirmed against "AI vs Traditional Cost Management_ What's the
// Difference_.docx" - same title/body, different wording). The real "Bridging the Gap"
// article already exists in the blogs table under a different slug, so this file is
// skipped rather than importing wrong content under its title.
const SKIP_FILES = new Set(["Bridging the Gap Between DevOps and Finance Teams with FinOps.docx"]);

// Content-planning category per article, from the source spreadsheet.
const CATEGORY_BY_FILE = {
  "How AI is Transforming Financial Operations (FinOps) in 2026.docx": "AI in Finance & Operations",
  "Predictive Cost Optimization_ Using AI to Forecast Cloud Spend.docx": "AI in Finance & Operations",
  "AI vs Traditional Cost Management_ What’s the Difference_.docx": "AI in Finance & Operations",
  "AI Token Optimization_ The Complete Guide to Cutting LLM Costs.docx": "AI in Finance & Operations",
  "How Engineering Teams Can Take Ownership of Cloud Costs.docx": "DevOps + FinOps (FinOps Culture)",
  "Cloud Cost Optimization Strategies for SaaS Companies.docx": "Industry-Specific Use Cases",
  "How E-commerce Businesses Can Control Seasonal Cloud Spending.docx": "Industry-Specific Use Cases",
  "FinOps for Healthcare & Fintech_ Compliance + Cost Efficiency.docx": "Industry-Specific Use Cases",
  "Top FinOps Tools in 2026_ Features, Pricing & Comparison.docx": "Bonus (High-Engagement Topics)",
  "Manual vs Automated Cloud Cost Management_ Which is Better_.docx": "Bonus (High-Engagement Topics)",
};
...
function articleFrom(file) {
  ...
  // The article ends at whichever comes first: the rule that separates it from a
  // trailing FAQ JSON-LD block, or a "FAQ Schema" heading that introduces one directly
  // (some source docs omit the separator rule). Either way the raw JSON-LD text must
  // never leak into the rendered content.
  let end = blocks.findIndex((b, i) => i > start && b.kind === "p" && /^[—–-]{2,}[\s—–-]*$/.test((b.text || "").trim()));
  const schemaEnd = blocks.findIndex((b, i) => i > start && b.kind === "p" && /^faq\s*schema\b/i.test((b.text || "").trim()));
  if (schemaEnd !== -1 && (end === -1 || schemaEnd < end)) end = schemaEnd;
  ...
  return {
    ...
    category: CATEGORY_BY_FILE[file] || "FinOps & Cloud Cost Optimization",
  };
}
...
async function main() {
  const files = fs
    ...
    .filter((f) => f.endsWith(".docx") && !f.startsWith("~$") && !SKIP_FILES.has(f))
    .sort();
  ...
  for (const skipped of SKIP_FILES) console.log(`SKIPPED (mismatched content): ${skipped}`);
  ...
  for (const a of articles) {
    const { rows } = await pool.query(
      ...
      [a.title, a.slug, a.excerpt, a.content, a.category, JSON.stringify(a.faqs), a.seoTitle, a.seoDescription]
    );
