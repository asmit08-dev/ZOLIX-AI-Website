...
export default async function BlogPage() {
  ...
  return <main className="min-h-screen bg-white px-6 pb-28 pt-40"><div className="mx-auto max-w-7xl">
      ...
      {blogs.length === 0 ? <div className="rounded-[36px] border border-dashed border-zolix-dark/15 bg-zolix-beige/50 px-8 py-24 text-center"><h2 className="text-2xl font-bold">New stories are on their way.</h2><p className="mt-3 text-zolix-dark/55">Check back soon for the latest from ZOLIX.</p></div> : <div className="grid gap-7 md:grid-cols-2 lg:grid-cols-3">{blogs.map((blog) => <article key={blog.id} className="group flex overflow-hidden rounded-[32px] border border-zolix-dark/10 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl"><Link href={`/blog/${blog.slug}`} className="flex w-full flex-col"><div className="relative h-52 overflow-hidden bg-zolix-dark"><img src={blog.coverImage.url} alt={blog.coverImage.alt} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />{blog.featured && <span className="absolute left-4 top-4 rounded-full bg-zolix-orange px-3 py-1 text-[9px] font-black uppercase tracking-widest text-white">Featured</span>}</div><div className="flex flex-1 flex-col p-7"><div className="mb-4 flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-zolix-orange"><span>{blog.category}</span><span className="inline-flex items-center gap-1 text-zolix-dark/40"><CalendarDays size={12}/>{date(blog.publishedAt)}</span></div><h2 className="text-2xl font-bold leading-tight tracking-tight">{blog.title}</h2>{blog.subtitle && <p className="mt-3 line-clamp-2 text-sm leading-relaxed text-zolix-dark/55">{blog.subtitle}</p>}<div className="mt-7 flex items-center gap-2 text-xs font-bold text-zolix-dark/45"><Heart size={14}/>{blog.likes} {blog.likes === 1 ? "like" : "likes"}</div></div></Link></article>)}</div>}
    </div></main>;
}
